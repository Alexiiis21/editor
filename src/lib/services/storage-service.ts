...
export class StorageService {
  ...
  private async ensureUploadDir() {
    try {
      ...
      await mkdir(path.join(this.uploadDir, 'chunks'), { recursive: true });
    } catch (error) {
  ...
  getPublicUrl(filepath: string): string {
    // Return API stream URL for authenticated video streaming
    const uploadDir = process.env.UPLOAD_DIR || path.join(process.cwd(), 'uploads');
    const rel = path.relative(uploadDir, filepath).split(path.sep).join('/');
    return '/api/stream/' + rel;
  }
  ...
  getFilePath(relativeOrAbsolutePath: string): string {
    // Si ya es una ruta absoluta, devolverla
    if (path.isAbsolute(relativeOrAbsolutePath)) {
      return relativeOrAbsolutePath;
    }
    // Si es relativa, combinarla con uploadDir
    return path.join(this.uploadDir, relativeOrAbsolutePath);
  }

  getUploadPath(type: 'videos' | 'thumbnails' | 'renders' | 'chunks' = 'videos'): string {
